...
export default class DateUtil {
    ...
    static FORMAT_DATE_MONTH_2 = "DD/MM"
    ...
    /**
     * Parse milliseconds to hour (hh:mm:ss)
     * @param {*} milliseconds 
     */
    static parseMillisecondToHour(milliseconds) {
        if (milliseconds > 0) {
            ...
            let minutes = parseInt(totalSeconds / 60, 10) % 60;
            let hours = parseInt(totalSeconds / 3600, 10);
            seconds = hours <= 0 ? seconds < 10 ? `0${seconds}${" "}${'giây'} ` : `${seconds}${" "}${'giây'} ` : '';
            minutes = minutes <= 0 ? '' : (minutes < 10 ? `0${minutes}${" "}${'phút'} ` : `${minutes}${" "}${'phút'} `);
            hours = hours <= 0 ? '' : (hours < 10 ? `0${hours}${" "}${'giờ'} ` : `${hours}${" "}${'giờ'} `);
            return `${hours}${minutes}${seconds}`
        }
        return { hours: '00', minutes: '00', seconds: '00' }
    }
